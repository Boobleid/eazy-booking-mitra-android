...
object UtilExceptions {
    ...
    fun Activity.handleApiError(
        failure: com.booble.reservasi.mitra.data.network.DataResource.Failure,
        retry: (() -> Unit)? = null
    ) {
        ...
        if (failure.isNetworkError) {
            ...
            try {
                val gson = Gson()
                val type = object : TypeToken<com.booble.reservasi.mitra.data.model.api.ErrorResponse>() {}.type
    ...
    fun Fragment.handleApiError(
        ...
        try {
            if (failure.isNetworkError) {
                ...
                val gson = Gson()
                val type = object : TypeToken<com.booble.reservasi.mitra.data.model.api.ErrorResponse>() {}.type
                val errorResponse: com.booble.reservasi.mitra.data.model.api.ErrorResponse? = gson.fromJson(failure.errorBody?.charStream(), type)
